module OnlineMeetingAgendaPatch
  ...
  def recordable?(by_user=nil)
    ...
    (! self.is_recording) && self.is_online? && user_condition && ((self.end_time_utc+(Setting[:plugin_redmine_online_meetings][:time_fix] || 0).to_i.minutes.to_i) > Time.now.utc.to_i)
  end
