#coding: utf-8
# SMSC.RU API (smsc.ru) версия 1.2 (07.06.2012)
...
class SMSC
	...
	attr_accessor :smsc_login
  attr_accessor :smsc_password
  #SMSC_LOGIN = "<login>"			# логин клиента
	...
	def send_sms_mail(phones, message, translit = 0, time = 0, id = 0, format = 0, sender = "")
		...
		Net::SMTP.start("send.smsc.ru") do |smtp|
			smtp.send_message("Content-Type: text/plain; charset=#{SMSC_CHARSET}\n\n#{self.smsc_login}:#{self.smsc_password}:#{id}:#{time}:#{translit},#{format},#{sender}:#{phones}:#{message}", SMTP_FROM, "send@send.smsc.ru")
		end
	...
	def _smsc_send_cmd(cmd, arg = "")
		url = (SMSC_HTTPS ? "https" : "http") + "://smsc.ru/sys/#{cmd}" + ".php?login=" + _urlencode(self.smsc_login) + "&psw=" + _urlencode(self.smsc_password) + "&fmt=1&charset=#{SMSC_CHARSET}&#{arg}"
